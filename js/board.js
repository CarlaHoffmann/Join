...
function addSearchTask() {
    const searchField = document.getElementById('searchField').value.toLowerCase().trim();
    const columns = ['toDo', 'progress', 'feedback', 'done'];
    ...
    columns.forEach(column => {
        ...
        tasks.forEach(task => {
            ...
            if (title.includes(searchField) || description.includes(searchField) || category.includes(searchField)) {
        ...
        // Placeholder anzeigen/verstecken basierend auf der Suche
        const placeholder = document.getElementById(column + 'Placeholder');
        placeholder.style.display = tasksFound ? 'none' : 'block';
    });

    // Keine Übereinstimmungen gefunden?
    const noSearchResult = document.getElementById('no-search-result');
    const totalVisibleTasks = document.querySelectorAll('.task-card[style*="display: block"]').length;
    noSearchResult.style.display = totalVisibleTasks === 0 ? 'block' : 'none';

    // Suchfeld löschen-Button anzeigen/verstecken
    const deleteSearch = document.getElementById('delete-search');
    deleteSearch.classList.toggle('d-none', searchField === '');
}

function deleteSearch() {
    const searchField = document.getElementById('searchField');
    searchField.value = '';
    addSearchTask(); // Filter zurücksetzen
}
